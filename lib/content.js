...
function normalizeAffiliateLinks(value) {
...
function normalizeAffiliateWidgets(value) {
  return toArray(value).map((entry) => ({
    key: String(entry?.key || ""),
    provider: String(entry?.provider || "").toLowerCase(),
    scriptSrc: String(entry?.scriptSrc || ""),
    label: String(entry?.label || ""),
    context: String(entry?.context || ""),
  }));
}
...
function normalizeMetadata(data) {
  return {
    ...
    breadcrumbLabel: data.breadcrumbLabel ? String(data.breadcrumbLabel) : null,
    ...
    heroIllustrative: data.heroIllustrative === true,
    featured: data.featured === true,
    draft: data.draft !== false,
    noindex: data.noindex !== false,
    affiliateDisclosure: data.affiliateDisclosure === true,
    affiliateKeys: toArray(data.affiliateKeys).map(String),
    primaryAffiliateKeys: toArray(data.primaryAffiliateKeys).map(String),
    affiliateLinks: normalizeAffiliateLinks(data.affiliateLinks),
    affiliateWidgets: normalizeAffiliateWidgets(data.affiliateWidgets),
    commercialPriority: Number.isInteger(data.commercialPriority)
      ? data.commercialPriority
      : null,
...
export function renderMarkdown(markdown) {
  ...
  Object.assign(renderer, {
    ...
    table(token) {
      return `<div class="table-scroll" role="region" aria-label="Scrollable comparison table" tabindex="0">${renderTable.call(this, token)}</div>`;
    },
  });
...
export async function getRelatedContent(relatedSlugs, excludeSlug = null) {
  if (!relatedSlugs?.length) return [];
  const entries = await getAllContent();
  return relatedSlugs
    .map((slug) => entries.find((entry) => entry.slug === slug))
    .filter((entry) => entry && !entry.draft && entry.slug !== excludeSlug);
}

export function isPublishedMonetizedContent(entry) {
  return Boolean(
    entry &&
    !entry.draft &&
    !entry.noindex &&
    (entry.affiliateLinks?.length || entry.affiliateWidgets?.length),
  );
}

export function sortCommercialContent(entries = []) {
  return [...entries].sort((a, b) => {
    const priorityA = Number.isInteger(a.commercialPriority) ? a.commercialPriority : Number.MAX_SAFE_INTEGER;
    const priorityB = Number.isInteger(b.commercialPriority) ? b.commercialPriority : Number.MAX_SAFE_INTEGER;
...
export async function getCountryParams() {
  const entries = await getAllContent();
  return entries
    .filter((entry) => !entry.draft && entry.contentType === "country")
...
export async function getCityParams() {
  const entries = await getAllContent();
  return entries
    .filter((entry) => !entry.draft && entry.contentType === "city")
...
export async function getArticleParams() {
  const entries = await getAllContent();
  return entries
    .filter((entry) => !entry.draft && entry.contentType === "article")
    .map((entry) => ({
