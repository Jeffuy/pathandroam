...
import { consentStorageKey, updateGoogleConsent, clearAnalyticsCookies } from "./google-consent.js";
...
function subscribe(callback) {
  ...
  function sync(event) {
    ...
    updateGoogleConsent(next?.analytics === true);
    if (!next?.analytics) {
      setAnalyticsEnabled(false);
      clearAnalyticsCookies();
    }
    if (previous?.affiliates && !next?.affiliates) {
...
export function saveTrackingConsent({ analytics, affiliates }) {
  ...
  // Unload previously accepted booking widgets; analytics-only changes need no reload.
  if (previous?.affiliates && !affiliates) {
    window.location.reload();
    return;
  }
